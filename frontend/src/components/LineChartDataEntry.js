...
const LineChartDataEntry = () => {
    ...
    for (let i = 0; i < elecConsumptions.length; i++) {
        elecConsumptions[i] = elecConsumptions[i] * 0.386
    }
    // Water: 0.00262/gallon(Source: https://awwa.onlinelibrary.wiley.com/doi/full/10.5942/jawwa.2012.104.0064)
    for (let i = 0; i < waterConsumptions.length; i++) {
        waterConsumptions[i] = waterConsumptions[i] * 0.00262
    }
    // Gas: 8.50/gallon (Source: https://www.eia.gov/environment/emissions/co2_vol_mass.php)
    for (let i = 0; i < gasConsumptions.length; i++) {
        gasConsumptions[i] = gasConsumptions[i] * 8.5
    }
...
export default LineChartDataEntry;